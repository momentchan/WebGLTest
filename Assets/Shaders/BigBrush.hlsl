...
void DrawBigBrush_float(float2 UV, float2 Center, float Count, float Width, float Height, float ratio, float strength, out float Out)
{
    ...
    for (int i = 0; i < Count; i++)
    {
        ...
        float offset = 0;//snoise(float2(n * 34.5, UV.y*2)) * 0.05;
...
float DrawSingle(float2 UV, float n, float Width, float Height, float ratio, float strength)
{
    float d = length(UV);
        
    float offset = snoise(float2(n * 123.45, UV.y * 4))*0.1;
        
    float2 size = frac(float2(n * 43.4, n * 123.8));
        
    float x = lerp(-1, 1, n);
    //x = 0;
    float y = lerp(-0.2, 0.2, frac(n * 133));
    //y = 0;
    float w = lerp(0.5, 1.5, size.x) * Width;
   // float w =  Width;
    float h = lerp(0.3, 1, size.y) * Height;
    
    float intensity = lerp(0.1, 1, frac(n * 345.5));
    //float h = Height;
    ...
    float r = ratio * speed;
    float fade = 1 - smoothstep(delay + r, delay + r + 0.05, UV.y+0.5);
        
    return saturate(drawEllipse(UV + offset, float2(x, y), w, h)) * strength * intensity * fade;
}

void DrawBigBrushNew_float(float2 UV, float2 id, float Width, float Height, float ratio, float strength, out float Out)
{
    Out = 0;
    for (int x = -10; x <= 10; x++)
    {
        float2 offs = float2(x, 0);
        
        float n = nrand(id + offs);
        Out += DrawSingle(UV - offs, n, Width, Height, ratio, strength) / 21;
    }
}